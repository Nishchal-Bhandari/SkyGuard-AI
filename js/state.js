/**
 * MONSOON SENTINEL - REACTIVE APPLICATION STATE
 */
...
class AppState {
  constructor() {
    ...
    this.currentRole = isAuthed ? auth.getRole() : "admin";
    this.activeStationId = (isAuthed && auth.getAssignedStationId()) ? auth.getAssignedStationId() : "AWS-07";
    this.currentView = (isAuthed && this.currentRole === "station_operator") ? "station-hud" : "command-center";
    this.stations = JSON.parse(JSON.stringify(SEED_STATIONS));
    this.incidents = JSON.parse(JSON.stringify(SEED_INCIDENTS));
    this.history = {}; // station_id -> array of time-series points
    this.offlineBuffer = [];
    this.isOfflineMode = false;
    
    // Physics & QC Threshold Configuration
    this.qcConfig = {
      temp_min: -20,
      temp_max: 55,
    ...
    // Model Governance & Model Registry (per DOCS/skill.md)
    this.activeModelId = "iforest-v1.4";
    this.modelRegistry = [
      {
        id: "iforest-v1.4",
    ...
    // Live Model Drift & Health Monitoring Metrics
    this.modelDrift = {
      drift_score: 2.1, // percentage drift
      drift_status: "NOMINAL", // NOMINAL, WARNING, DRIFT_DETECTED
      ood_sensor_flags: 0,
      false_positive_feedback_rate: 1.8, // % from operator reviews
      score_distribution: [
    ...
    // External Meteorological Context Lineage (IMD Radar / Reanalysis)
    this.externalDataLineage = [
      {
    ...
    // Maintenance Checklist state per station
    this.checklists = {
      "AWS-07": [
  ...
  initHistory() {
    // Generate 20 historical points for each station
    const now = Date.now();
  ...
  setView(viewName) {
    // RBAC Route Guard: Station Operator cannot access Admin pages
    const adminOnlyViews = ['command-center', 'fleet-map', 'incidents', 'qc-rules', 'fault-lab', 'model-governance', 'credentials', 'export'];
    if (this.currentRole === "station_operator" && adminOnlyViews.includes(viewName)) {
      this.currentView = "station-hud";
      this.notify();
      return;
    }
