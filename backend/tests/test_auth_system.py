...
class TestSkyGuardAuthSystem(unittest.TestCase):
    ...
    def test_01_database_and_admin_initialization(self):
        """Test SQLite schema initialization and default admin seeding"""
        with get_db() as conn:
            cursor = conn.cursor()
    ...
    def test_03_admin_create_station_and_uniqueness(self):
        ...
        req = CreateStationRequest(
            station_id="AWS-07",
            station_name="Hyderabad Deccan Plateau",
        ...
        self.assertEqual(created.station_id, "AWS-07")
        self.assertEqual(created.username, "operator_hyd")
        ...
        with get_db() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT password_hash FROM stations WHERE station_id = 'AWS-07'")
    ...
    def test_04_station_login_and_deactivation(self):
        ...
        # Login with username
        res = login_station(StationLoginRequest(username="operator_hyd", password="securePassword@2026"))
        self.assertTrue(res.success)
        self.assertEqual(res.role, "station_operator")
        self.assertEqual(res.user.assignedStationId, "AWS-07")
        
        # Login with station_id
        res2 = login_station(StationLoginRequest(username="AWS-07", password="securePassword@2026"))
        ...
        with self.assertRaises(HTTPException) as ctx:
            login_station(StationLoginRequest(username="operator_hyd", password="securePassword@2026"))
        self.assertEqual(ctx.exception.status_code, 403)
        
        # Reactivate station
        toggle_station_status("AWS-07", StatusToggleRequest(status="ACTIVE"), admin_user=admin_user)
        res3 = login_station(StationLoginRequest(username="operator_hyd", password="securePassword@2026"))
    ...
    def test_05_station_identity_isolation(self):
        ...
        create_station(CreateStationRequest(
            station_id="AWS-12",
            station_name="Mumbai Coastal Radar",
            username="operator_mum",
            password="mumbaiPassword@2026",
            latitude=18.9220,
            longitude=72.8346,
            elevation=14.0,
            region="West Coast",
    ...
    def test_06_admin_list_and_password_reset(self):
        ...
        self.assertGreaterEqual(len(stations_list), 2)
        ...
        reset_res = reset_station_password("AWS-07", ResetPasswordRequest(new_password="newResetPass@2026"), admin_user=admin_user)
        ...
        new_login = login_station(StationLoginRequest(username="operator_hyd", password="newResetPass@2026"))
